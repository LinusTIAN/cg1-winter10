...
bool CGSkelStoreData(IPObjectStruct *PObj)
{
	...
	if (PObj -> ObjType != IP_OBJ_POLY) {
		// Non-polygonal object - ignore silently
		return true;
	}
	...
	{
		for (int j=0; j<3; j++)
			newObj->m_color[j] = RGB[j];
	}
	else{
		for (int j=0; j<3; j++)
			newObj->m_color[j] = 1.0;
	}
	...
	for (PPolygon = PObj -> U.Pl; PPolygon != NULL;	PPolygon = PPolygon -> Pnext) 
	{
			if (PPolygon -> PVertex == NULL) {
				AfxMessageBox("Dump: Attempt to dump empty polygon");
				delete newObj;
				return false;
			}
			...
			MyPolygon *poly = new MyPolygon(i);
			...
			/* Close the polygon. */
			newObj->AddPolyRef(poly);
	}

	objectData->addObjectRef(newObj);
	/* Close the object. */
	return true;
}
